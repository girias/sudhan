...
public class InvoiceDAO {
  ...
  public Integer invoiceCreation(com.java.example.JavaSBA_InvoiceProcess.Invoice invoiceObj) {
    try {
      ...
      //            String sql2 = "Select id from invoice where invoice_number = '" +
      // invoiceObj.getInvoiceNumber() + "'";
      ...
      while (rs1.next()) {
        ...
        for (User user : users) {
          if (user.getId().equals(rs1.getInt("user_id"))) {
            com.java.example.JavaSBA_InvoiceProcess.Invoice inv =
                new com.java.example.JavaSBA_InvoiceProcess.Invoice(
                    id, inv_num, status, amount, created, user);
            invoices.add(inv);
          }
        }
      }
  ...
  public void updateInvoiceStatus(
      com.java.example.JavaSBA_InvoiceProcess.Invoice invoiceObj, String status) {
    try {
      Connection dbconn = com.java.example.JavaSBA_InvoiceProcess.DbConnection.getConnection();
      Statement stmt = dbconn.createStatement();
      String sql =
          "Update invoice set status ='" + status + "' where id = '" + invoiceObj.getId() + "'";
