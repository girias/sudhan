...
public class Main {
  ...
  public static void main(String[] Args) throws Exception {
    ...
    // System.out.println("com.java.example.Invoice Id      Item Name        Quantity        Price
    //         Total Amount");
    System.out.format(
        "%-15s %-15s  %-15s %-15s %-15s\n",
    ...
    for (int i = 0; i < details.getInvoiceLine().length; i++) {
      System.out.println(
          details.getId()
              + "      "
              + invoiceLine[i].getItem().getName()
              + "      "
              + invoiceLine[i].getQuantity()
              + "      "
              + invoiceLine[i].getItem().getPrice()
              + "      "
              + invoiceLine[i].getAmount()
              + "      ");
    }
